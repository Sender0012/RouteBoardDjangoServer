...
export {}; // Mark this as a module to avoid global‐scope collisions

type Dot = { row: number; col: number; color: string };
type Line = { from: Dot; to: Dot };

const gridContainer = document.getElementById("gridContainer")!;
const linesInput = document.getElementById("linesInput") as HTMLInputElement;
const form = document.getElementById("pathForm") as HTMLFormElement;
const colorSelector = document.getElementById("lineColorSelector") as HTMLSelectElement;

declare const initialLines: Line[];
declare const initialDots: Dot[];
declare const boardRows: number;
declare const boardCols: number;

let selectedStart: { row: number; col: number } | null = null;
let currentLineColor: string = "";
let dots: Dot[] = [...initialDots];
let lines: Line[] = [...initialLines];

function dotKey(r: number, c: number): string {
  return `${r}-${c}`;
}

function getDotAt(r: number, c: number): Dot | undefined {
  return dots.find(d => d.row === r && d.col === c);
}

function cellOnLine(line: Line, r: number, c: number): boolean {
  const { from, to } = line;
  // Horizontal?
...
function highlightCell(r: number, c: number, on: boolean): void {
...
function drawGridAndLines(): void {
...
function populateColorSelector(): void {
